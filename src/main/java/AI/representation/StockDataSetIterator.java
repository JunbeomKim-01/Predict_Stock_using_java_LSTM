...
public class StockDataSetIterator implements DataSetIterator{
    private final Map<PriceCategory, Integer> featureMapIndex = ImmutableMap.of(PriceCategory.OPEN, 0, PriceCategory.HIGH, 1,
            PriceCategory.LOW, 2, PriceCategory.CLOSE, 3, PriceCategory.VOLUME, 4);
    ...
    public StockDataSetIterator (int miniBatchSize, int exampleLength, double splitRatio, PriceCategory category) {
        List<StockData> stockDataList = readStockData();
    ...
    private List<StockData> readStockData() {
...
//            Storage.getIteam("STOCKNAME")
            stockDataList.add(new StockData(arr[0],"삼성전자", nums[0], nums[1], nums[2], nums[3], nums[4]));
        }
